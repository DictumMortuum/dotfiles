...
@drupal() {
  case $1 in
    make) __make $2 $3;;
    install) __install $2 $3;;
  esac
}

__make() {
  if [[ $# == 2 ]]; then
    cd $DRUPAL/make-sportsbook-pmu_sportsbook && git checkout $2
  fi
...
__install() {
  ...
  endpoint_configure_form.openbet_api_trusted_token="$token" \
  ...
  endpoint_configure_form.siteserver_url="http://dev01.openbet/pmu/$2/ss/" \
  endpoint_configure_form.openbet_api_url="http://dev01.openbet/pmu/$2/oxi" \
